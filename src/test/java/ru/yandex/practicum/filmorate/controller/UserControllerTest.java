...
class UserControllerTest {
    ...
    void shouldCreateNormalUser() {
        User user = new User(0, "user@email.ru", "UserLogin", "User Name", LocalDate.of(1980, 5, 25), new HashSet<>());
    ...
    void shouldSetLoginAsNameOfUser() {
        User user = new User(0, "user@email.ru", "UserLogin", "", LocalDate.of(1980, 5, 25), new HashSet<>());
        User returnedUser = userController.create(user);
    ...
    void shouldNotCreateWrongEmail() {
        User wrongUser1 = new User(0, "", "UserLogin", "", LocalDate.of(1980, 5, 25), new HashSet<>());
        User wrongUser2 = new User(0, "user-email.ru", "UserLogin", "", LocalDate.of(1980, 5, 25), new HashSet<>());
    ...
    void shouldNotCreateWrongLogin() {
        User wrongUser1 = new User(0, "user@email.ru", "", "", LocalDate.of(1980, 5, 25), new HashSet<>());
        User wrongUser2 = new User(0, "user@email.ru", "User Login", "", LocalDate.of(1980, 5, 25), new HashSet<>());
        RuntimeException exception1 = assertThrows(RuntimeException.class, () -> userController.create(wrongUser1));
        RuntimeException exception2 = assertThrows(RuntimeException.class, () -> userController.create(wrongUser2));
    ...
    void shouldIgnoreBdInFuture() {
        RuntimeException exception = assertThrows(RuntimeException.class, () -> userController.create(new User(0, "user@email.ru", "UserLogin", "", LocalDate.of(3000, 5, 25), new HashSet<>())));
        assertEquals("Дата рождения не может быть в будущем.", exception.getMessage());
    }

    @Test
    void shouldUpdateNormalUser() {
        userController.create(new User(0, "user@email.ru", "UserLogin", "User Name", LocalDate.of(1980, 5, 25), new HashSet<>()));
        User expectedUser = new User(1, "newUserEmail@email.ru", "newUserLogin", "NewUser Name", LocalDate.of(1980, 5, 25), new HashSet<>());
    ...
    void shouldNotUpdateIfIdIsWrong() {
        userController.create(new User(0, "user@email.ru", "UserLogin", "User Name", LocalDate.of(1980, 5, 25), new HashSet<>()));
        User expectedUser = new User(2, "UpdatedUserEmail@email.ru", "UpdatedUserLogin", "UpdatedUser Name", LocalDate.of(1980, 5, 25), new HashSet<>());
